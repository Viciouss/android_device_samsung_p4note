...
#define LOG_TAG "P4NoteGatekeeper"
...
using p4note::SoftGateKeeperDevice;

struct p4note_gatekeeper_device {
    gatekeeper_device device;
    SoftGateKeeperDevice *s_gatekeeper;
};

static p4note_gatekeeper_device s_device;
...
static int enroll(const struct gatekeeper_device *dev __unused, uint32_t uid,
    ...
    SoftGateKeeperDevice *s_gatekeeper = ((p4note_gatekeeper_device*)(dev))->s_gatekeeper;
...
static int p4note_gatekeeper_open(const hw_module_t *module, const char *name,
        hw_device_t **device) {
